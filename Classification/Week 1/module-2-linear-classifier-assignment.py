...
#simple if else sentiment review tool to iterate through large set(apply method)
print scores[0]
print type(scores)
...
test_data['probaility'] = sentiment_model.predict(test_data, output_type='probability')
top_20 = test_data.topk('probability', k=20, reverse=False)
print top_20
top_20.show()
...
def get_classification_accuracy(model, data, true_labels):
    ...
    prediction = scores.apply(lambda x: 1 if x>0 else -1)
    
    # Compute the number of correctly classified examples
    ## YOUR CODE HERE
    correct= 0 
    for i in range(len(data)): #loop through all of the data
        if prediction[i] == true_labels[i]: # 1 or -1
            correct+=1
...
simple_model.coefficients[simple_model.coefficients['value']>=0].sort('value',ascending=False)
