...
x = (len(safe_loans_raw) + len(risky_loans_raw))
type(x)
print (float(len(safe_loans_raw)) / (len(safe_loans_raw) + len(risky_loans_raw)))
...
x = small_model.predict(sample_validation_data)
len(sample_validation_data)
...
y = small_model.predict(sample_validation_data, output_type='probability')
print y
