...
def reached_minimum_node_size(data, min_node_size):
    ...
    ## YOUR CODE HERE
    if (len(data) <= min_node_size): #this will check if the data split will have more or less
        return True
    else:
        return False
...
def error_reduction(error_before_split, error_after_split):
    ...
    ## YOUR CODE HERE
    return (error_before_split - error_after_split) # positive means before has more error - > split
...
def intermediate_node_num_mistakes(labels_in_node):
    ...
    #look at +1 under the label 'safe_loans'
    ...
    if positive_count > negative_count:
        return negative_count # because postive is majority
    else:
        return positive_count #negative is majority
...
def best_splitting_feature(data, features, target):
    ...
    for feature in features:
        #the binary split as needed after the feature engineering
...
def evaluate_classification_error(tree, data):
    ...
    ## YOUR CODE HERE
    for i in range(len(prediction)): # loop through all the predictions and actual to find # of mistakes
        if prediction[i] != data['safe_loans'][i]:
            mistakes +=1
    accuracy = (mistakes) / float(len(data))
    return accuracy
...
print "Number of nodes in model 1:", count_leaves(model_7)
print "Number of nodes in model 2:", count_leaves(model_8)
print "Number of nodes in model 3:", count_leaves(model_9)
